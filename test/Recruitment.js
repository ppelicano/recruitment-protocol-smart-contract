...
describe("Recruitment contract", function () {
  ...
  it("Final payment with wrong initial deposit index fails", async function () {
    ...
    await tInstance
      .connect(addr1)
      .approve(rInstance.address, `24000${"0".repeat(18)}`);
    expect(
      await rInstance
        .connect(addr1)
        .setFinalDeposit(DAI, `24000${"0".repeat(18)}`, 1)
    ).to.revertedWith("Initial deposit index does not match!");
  });

  // company setting scores for a candidate
  it("A Company should be able to send scores to a candidate", async function () {
    const { rInstance, tInstance, owner, addr1, addr2, DAI } =
      await loadFixture(deployRecruitmentFixture);

    await rInstance
      .connect(addr1)
      .submitReferralScore(1, owner.address, addr1.address, "hello@gmail.com");
